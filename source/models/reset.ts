...
passwordResetSchema.methods.isValid = function (plaintextToken: string) {
    const passwordResetDocument = this as PasswordResetDocument;
    const hash = PasswordReset.hashedToken(plaintextToken);
    return timingSafeEqual(Buffer.from(hash), Buffer.from(passwordResetDocument.token)) && passwordResetDocument.expiredAt > new Date();
};
